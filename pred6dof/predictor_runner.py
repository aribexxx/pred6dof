...
class KalmanRunner():
    ...
    def run_with_single_pred_win(self,measure_motion,w):
          # Adjust F depending on the lookahead time
        print("Prediction window = %s ms" % (w * 1e-3))
        w_sec = w * 1e-3
        f_l = np.array([[1.0, w_sec], [0.0, 1.0]])
        setattr(self.kf, 'F_lookahead', block_diag(f_l, f_l, f_l, f_l, f_l, f_l, f_l))
        xs, covs, x_preds = [], [], []
        ...
        xs.append(self.kf.x)
        covs.append(self.kf.P)
        x_preds = np.squeeze(self.kf.x_pred)
        self.z_prev = current_measure_z
        print(f"origin:{current_measure_z}\n pred:{x_preds}")
